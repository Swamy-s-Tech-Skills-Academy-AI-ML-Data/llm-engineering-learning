# Week 11: Capstone Build

**Theme:** Capstone Build  
**Time Commitment:** 5 days × 30 minutes = 2.5 hours  
**Primary Outcome:** Integrated RAG + tools + eval + Capstone repo folder  
**Platforms:** OpenAI + Azure OpenAI (Python, Go, Node.js, **React intro**)

---

## 🎯 Week 11 Learning Objectives

By the end of this week, you will:
- [ ] Build a complete capstone project
- [ ] Integrate RAG, tools, and evaluation
- [ ] Combine all learned concepts
- [ ] Create production-ready system
- [ ] Document and showcase project
- [ ] **Introduction to React**: Optional frontend component (if time permits)

---

## 📅 Day-by-Day Breakdown

### Day 1: Capstone Planning (30 minutes)

**Learning Goal:** Plan your capstone project

**Tasks (30 min):**

1. **Choose Capstone** (10 min)
   - Domain Q&A Assistant (RAG)
   - Multi-Source Summarizer
   - Agentic Research Helper
   - Structured Data Extractor

2. **Define Architecture** (10 min)
   - Components needed
   - Integration points
   - Technology stack

3. **Create Project Plan** (10 min)
   - Milestones
   - Deliverables
   - Timeline

**Exercise:**
- [ ] Document capstone plan
- [ ] Set up project structure
- [ ] Define requirements

---

### Day 2: Core Implementation (30 minutes)

**Learning Goal:** Build core capstone features

**Tasks (30 min):**

1. **Implement Core** (20 min)
   - RAG system
   - Tool integration
   - Agent logic

2. **Integration** (10 min)
   - Connect components
   - Test integration
   - Fix issues

**Exercise:**
- [ ] Core features working
- [ ] Components integrated
- [ ] Basic testing complete

---

### Day 3: Advanced Features (30 minutes)

**Learning Goal:** Add advanced features

**Tasks (30 min):**

1. **Add Features** (20 min)
   - Evaluation system
   - Error handling
   - Performance optimization

2. **Polish** (10 min)
   - UI improvements
   - Documentation
   - Code quality

**Exercise:**
- [ ] Advanced features complete
- [ ] System polished
- [ ] Ready for evaluation

---

### Day 4: Evaluation & Testing (30 minutes)

**Learning Goal:** Evaluate capstone

**Tasks (30 min):**

1. **Create Tests** (15 min)
   - Unit tests
   - Integration tests
   - Evaluation metrics

2. **Run Evaluation** (15 min)
   - Test all features
   - Measure performance
   - Document results

**Exercise:**
- [ ] Tests passing
- [ ] Evaluation complete
- [ ] Results documented

---

### Day 5: Documentation & Presentation (30 minutes)

**Learning Goal:** Document and present capstone

**Tasks (30 min):**

1. **Documentation** (15 min)
   - README
   - Architecture docs
   - Usage guide

2. **Final Review** (15 min)
   - Review all features
   - Week 11 reflection
   - Prepare for showcase

**Deliverable:**
- [ ] Complete capstone project
- [ ] All features working
- [ ] Comprehensive documentation
- [ ] Evaluation results

---

## 🔄 Next Week Preview

**Week 12:** Review & Deep Dives
- Retrospective
- Advanced experiments
- Lessons learned
- Next steps

